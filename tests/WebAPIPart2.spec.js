...
test.beforeAll(async ({browser})=>{
    ...
    await page.getByPlaceholder('email@example.com').fill('kovey86773@grassdev.com');
    await page.locator('input#userPassword').fill('NoPeaking>--<');
    await page.locator("[value='Login']").click();
    await page.waitForLoadState('networkidle');
    await context.storageState({path: 'loginDetails.json'});
    await page.waitForTimeout(5*1000);
    webContext = await browser.newContext({storageState: 'loginDetails.json'});
})

test('Client application', async()=>{
